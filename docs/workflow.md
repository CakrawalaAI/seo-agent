...
- Upsert rows into `website_keywords` with metrics columns.

4) Plan
- Build 30-day runway of `articles(status=queued)` from `website_keywords(include=true)`.
...
Jobs
- `crawl` → `discovery` → (optionally) `plan` → `generate` → `publish`.
