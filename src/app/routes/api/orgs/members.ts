...
import { organizationMembers } from '@entities/org/db/schema'
    ...
    handlers: {
      GET: safeHandler(async ({ request }) => {
        ...
        if (hasDatabase()) {
          try {
            const db = getDb()
            const rows = await db.select().from(organizationMembers).where(eq(organizationMembers.orgId, orgId)).limit(200)
