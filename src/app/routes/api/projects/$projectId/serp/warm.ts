...
export const Route = createFileRoute('/api/projects/$projectId/serp/warm')({
  server: {
    handlers: {
      POST: safeHandler(async ({ params, request }) => {
        ...
        const topM = Math.max(1, Math.min(200, Number(url.searchParams.get('topM') || '50')))
        ...
        if (queueEnabled()) {
          for (const k of list) {
            await publishJob({ type: 'serp', payload: { canonPhrase: k.phrase, language: project.defaultLocale || 'en-US', locationCode: loc, device, topK: 10 } })
            queued++
          }
        }
        return json({ queued })
