...
export const Route = createFileRoute('/api/schedules/metrics')({
  server: {
    handlers: {
      POST: safeHandler(async ({ request }) => {
        ...
        // Enqueue metrics job for current month and default location
        const month = new Date()
        const asOfMonth = `${month.getUTCFullYear()}-${String(month.getUTCMonth() + 1).padStart(2, '0')}`
        let queued = 0
        if (queueEnabled()) {
          for (const c of canons) {
            await publishJob({ type: 'metrics', payload: { canonPhrase: c.phraseNorm, language: c.languageCode, locationCode: 2840, month: asOfMonth } })
            queued++
          }
        }
        return json({ queued })
