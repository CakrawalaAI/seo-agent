  ...
  server: {
    handlers: {
      PUT: safeHandler(async ({ params, request }) => {
        ...
        if (hasDatabase()) { try { const db = getDb(); await db.update(articles).set({ plannedDate, updatedAt: new Date() as any }).where(eq(articles.id, params.planItemId)) } catch {} }
        const updated = await planRepo.updateDate(params.planItemId, plannedDate)
