...
import { loader as projectDetailLoader } from '../src/pages/projects/$projectId/loader'
...
describe('route loaders', () => {
  ...
  it('project detail primes core queries', async () => {
    const { qc, calls } = createFakeQC()
    await projectDetailLoader({ context: { queryClient: qc as any }, params: { projectId: 'proj_1' } } as any)
    ...
    expect(joined).toContain('crawlPages')
    expect(joined).toContain('keywords')
  })
