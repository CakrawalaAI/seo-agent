...
describe('link-graph API (memory path)', () => {
  it('returns nodes and edges from crawlRepo links', async () => {
    const projectId = 'proj_lg'
    crawlRepo.addOrUpdate(projectId, { url: 'https://ex.com/', depth: 0, status: 'completed', metaJson: { title: 'Home' }, extractedAt: new Date().toISOString() })
    crawlRepo.addOrUpdate(projectId, { url: 'https://ex.com/about', depth: 1, status: 'completed', metaJson: { title: 'About' }, linksJson: [{ href: 'https://ex.com/' }], extractedAt: new Date().toISOString() })
